...
class SubjectAssignService
{
    ...
    public function get()
    {
        ...
        return DB::table('academic_subject_assign as asa')
            ...
            ->leftJoin('academic_branch as ab', 'ab.id', 'asa.branch_id')
            ->leftJoin('academic_class as ac', 'ac.id', 'asa.class_id')
            ->leftJoin('academic_subject as sub', 'sub.id', 'asa.subject_id')
            ->where('asa.status_id', 1)
            ->groupBy('asa.class_id', 'asa.status_id', 'ab.branch_name', 'ab.id');
    ...
    public function edit($branch_id,$id)
    {
      ...
      ->leftJoin('academic_branch as br','asa.branch_id','br.id')
      ->leftJoin('academic_class as ac','asa.class_id','ac.id')
      ->where('asa.status_id', 1)
